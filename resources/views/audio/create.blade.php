...
@section('content')
    <div class="py-4">
        <h3 class="form-title fw-bold mb-2 pb-2 border-bottom">Tambah Audio</h3>

        <!-- Tombol Kembali dengan ikon -->
        <a href="{{ route('audio.index') }}" class="btn btn-sm btn-secondary mb-2">
        ...
        <div class="isi">
            <form action="{{ route('audio.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-2">
                    <label for="audio" class="form-label">Audio<span class="text-danger">*</span></label>
                    <div class="custom-file-upload">
                        <input type="file" name="audio" id="audio" class="form-control @error('audio') is-invalid @enderror" accept="audio/*" onchange="updateFileName(this)" style="display: none;">
                ...
                <a href="{{ route('audio.index') }}" class="btn btn-secondary mb-3">Batal</a>
            </form>
        </div>
    </div>
@endsection
