...
@section('content')
    <div class="edit-diary-py-4">
        <h3 class="edit-diary-fw-bold mb-2 pb-2 border-bottom">Tambah Diary</h3>
        ...
        <div class="edit-diary-isi">
            <form action="{{ route('diary.store') }}" method="POST">
                ...
                <div class="edit-diary-form-group">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" class="edit-diary-form-control @error('title') is-invalid @enderror" required>
                    @error('title')
                ...
                <div class="edit-diary-form-group">
                    ...
                    <textarea name="text" id="text" class="edit-diary-form-control @error('text') is-invalid @enderror" rows="5" required>{{ old('text') }}</textarea>
                    @error('text')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

                <button type="submit" class="btn">Simpan Diary</button>
            </form>
        </div>
    </div>
@endsection
